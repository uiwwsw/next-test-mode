...
test("Next server fetch call sites stay unchanged while console JSON refreshes actual SSR HTML", async ({ page, browser }) => {
  const errors = [];
  ...
  await expect(page.locator("[data-total]")).toHaveText("12.34", { timeout: 30000 });
  ...
  await page.waitForFunction(() => typeof window.test === "function");
  await page.evaluate(() => test.mock("/api/cart", { total: 3.14, items: [{ name: "Custom", price: 3.14 }] }));
