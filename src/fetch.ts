...
export const createMockFetch =
  ...
  async (input, init) => {
    ...
    return withAbort(signal, async () => {
      ...
      if (
        !mapRequest &&
        !testMode.hasMock(request) &&
        !testMode.hasPatch(request)
      ) {
        return originalFetch(input, init);
      }
      request = {
        ...request,
        body: await readRequestBody(input, init, request.method),
      };
      ...
      const patched = await testMode.applyPatch({
        ...request,
...
export const installMockFetch = (
  ...
  const replacement: typeof fetch = (input, init) =>
    installation.active
      ? wrapped(input, init)
      : previous.call(target, input, init);
