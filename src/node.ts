...
import { isTestModeBypassed } from "./internal/server-context.js";
...
const installations = new WeakMap<
  typeof fetch,
  { previous: typeof fetch; active: boolean }
>();
...
export type InstallServerTestModeOptions = Omit<
  WithTestModeOptions,
  "enabled"
> &
  Readonly<{
    ...
    getRequest: () =>
      ...
      | null
      | Promise<ServerRequestContext | null>;
...
export const installServerTestMode = ({
  ...
  if (!descriptor?.configurable || !("value" in descriptor))
    throw new TypeError(
      "Automatic server test mode requires a configurable fetch value. Install it once before other fetch accessors.",
    );
  const bypass = new AsyncLocalStorage<boolean>();
  const scopes = new WeakMap<
    object,
    ReturnType<typeof createServerTestMode>["runtime"]
  >();
  ...
  const wrap = (original: typeof fetch): typeof fetch => {
    ...
    const proxy = new Proxy(original, {
      apply(target, receiver, args: Parameters<typeof fetch>) {
        const forward = () =>
          Reflect.apply(target, receiver, args) as ReturnType<typeof fetch>;
        if (
          !active ||
          isTestModeBypassed() ||
          bypass.getStore() ||
        ...
        return bypass.run(true, async () => {
          ...
          if (!runtime) {
            runtime = createServerTestMode({
              ...options,
              enabled: true,
              ssr,
              cookieHeader: request.cookieHeader,
              originalFetch: target.bind(receiver),
            }).runtime;
          ...
          return createMockFetch(runtime, {
            cookieHeader: false,
            originalFetch: target.bind(receiver),
          })(input, init);
        });
...
export type WithTestModeOptions = Omit<TestModeOptions, "enabled"> &
  Readonly<{
...
export const withTestMode = <T extends Handler>(
  handler: T,
  {
    enabled = () =>
  ...
  const wrapped = function (
    ...
    if (
      !installation.active ||
      !(typeof enabled === "function" ? enabled(request) : enabled)
    )
      return requests.run(undefined, () =>
