...
이 페이지는 **브라우저 fetch / CSR**을 보여줍니다. 같은 명령으로 서버 HTML까지 바꾸는 **[공개 SSR 데모](https://test-mode-tau.vercel.app/api/ssr)**도 있습니다. [SSR 실행 예제](../server)와 [CSR / SSR 연결 가이드](../../docs/server-rendering.md)를 참고하세요.
...
CSR 샘플 API는 공개 JSON 파일입니다. 별도 `/api/ssr` Node 함수가 실제 SSR 데모를 제공합니다. Mock은 네트워크 요청을 생략하고, Patch는 원래 JSON을 받은 뒤 필드를 변경합니다. 입력한 값은 서버에 전송하거나 보관하지 않습니다.
...
콘솔 명령 후 자동 재요청은 **이 데모 앱의 구독 코드**입니다. 라이브러리를 설치한 다른 앱에서는 `setupTestMode({ refresh: ... })` 또는 `runtime.subscribe()`로 재요청을 연결할 수 있습니다. SSR 동기화 모드는 자동으로 페이지를 새로고침합니다. 앱 모양에 맞지 않는 JSON도 입력할 수 있으며, 이때 원본 응답과 앱의 처리 실패 상태가 함께 표시됩니다.
